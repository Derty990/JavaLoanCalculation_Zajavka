...
public class InputDataRepository {
    ...
    public Optional<InputData> read() {
        ...
        return Optional.of(InputData.builder()
                ...
                .wiborPercent(Optional.ofNullable(inputData.get("wiborPercent")).map(BigDecimal::new).orElseThrow())
                ...
                .marginPercent(Optional.ofNullable(inputData.get("marginPercent")).map(BigDecimal::new).orElseThrow())
                .overpaymentProvisionPercent(Optional.ofNullable(inputData.get("overpaymentProvisionPercent")).map(BigDecimal::new).orElseThrow())
    ...
    private static Map<String, List<String>> readFile() {
        try {
            ...
            log.error("Error loading data, interrupting. ", e);
            return Map.of();
        }
